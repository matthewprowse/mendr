...
- [x] **C1. get_user_id_by_email is callable by anonymous users**
...
- [x] **C2. user_home_stats returns any user's diagnoses and home addresses**
...
- [x] **H4. RLS is disabled on three public tables**
...
- [x] **H2. run_data_layer_maintenance is callable by anonymous users and deletes data**
...
- [x] **H6. Two SECURITY DEFINER views bypass RLS**
...
- [x] **C4. IDOR, GET api/diagnoses/[id] leaks any user's address, GPS, and photos**
...
- [x] **C5. IDOR and ownership hijack, PATCH api/diagnoses/[id]**
...
- [x] **C6. Unauthenticated personal data read and delete of provider applications**
...
- [x] **H5. Unauthenticated writes to arbitrary providers**
...
- [x] **M1. WhatsApp simulator is a public unauthenticated bot driver**
...
- [x] **M2. Open paid API proxies bypass the beta gate**
...
- [x] **M3. Rate limiting degrades silently to per lambda memory if Upstash env is missing**
...
- [x] **M4. Kill switches are not gated to non production**
...
- [x] **M5. Legacy admin login has no brute force limit and a non constant time compare**
...
- [x] **M6. Admin email routes trust client supplied recipients**
...
- [x] **M7. PostgREST filter string injection**
...
- [x] **M8. audit_logs INSERT policy is always true**
...
- [x] **M9. Nine functions have a mutable search_path**
...
- [x] **M11. Password change rotates the session as a side effect**
...
- [x] **M12. No shared authenticate and authorize helper**
...
- [x] **M13. Validation is hand rolled and inconsistent**
...
- [x] **M14. Unindexed foreign keys and per row auth in RLS**
...
- [x] **L1. pg_trgm extension lives in the public schema.** Move it to an extensions schema.
- [x] **L2. beta-access master code is compared in non constant time** with a per IP throttle only. Use a constant time compare and a per code attempt cap.
- [x] **L3. Anonymous analytics writes accept a client supplied session_id** in api/providers/[id]/view and api/events, allowing metric inflation. Relevant given the published stat guardrails.
- [x] **L4. Stale ADMIN_PASSWORD comments** appear in many route headers although the value is unused. Remove them.
- [x] **L5. cron-auth and some token checks use a plain equals** rather than a constant time compare. Negligible over the network, easy to harden.
- [ ] **L6. Two Supabase MCP servers are connected** and one returns no projects. Prune to one to keep tooling unambiguous.
- [x] **L7. Remove dead code** once the above land, the legacy admin login path, the two backup tables, and the stale environment comments.
...
Status legend: [x] done, [~] partial / blocked (see note), [ ] not done.

| ID | Severity | Area | Item | Done |
| --- | --- | --- | --- | --- |
| C1 | C | Database | Revoke anon execute on get_user_id_by_email | [x] |
| C2 | C | Database | Revoke and rewrite user_home_stats to use auth.uid (kept service-role-only) | [x] |
| C3 | C | Storage | Make diagnosis, gallery, message-attachments, vault private with signed URLs | [~] limits on all 8; vault + message-attachments private; diagnosis/gallery signed-URL migration deferred (needs running-app verification, see memory) |
| C4 | C | Route | Ownership check on GET api/diagnoses/[id] | [x] |
| C5 | C | Route | Ownership check on PATCH api/diagnoses/[id], drop client user_id | [x] |
| C6 | C | Route | Auth gate read and delete on application-progress and application-session | [x] |
| H1 | H | Storage | Verify diagnosis image URLs only issued signed to owner | [~] deferred with C3 full migration; C4 IDOR (the exploit vector) is closed |
| H2 | H | Database | Revoke anon execute on run_data_layer_maintenance | [x] |
| H3 | H | Quota | Revoke increment_diagnosis_quota, fix anon key and first message logic | [~] revoke + IP-keyed anon quota done; client-history first-message bypass needs a server-issued conversation token (noted in code) |
| H4 | H | Database | Enable RLS on ai_call_log, drop the two backup tables | [x] |
| H5 | H | Route | Ownership checks on provider gallery, sync, clean-profile, restore-token | [x] |
| H6 | H | Database | Recreate the two SECURITY DEFINER views as security_invoker | [x] |
| M1 | M | Abuse | Gate the WhatsApp simulator off in production | [x] |
| M2 | M | Cost | Global spend circuit breaker on the Google proxies | [x] |
| M3 | M | Cost | Make Upstash mandatory and fail closed in production | [x] |
| M4 | M | Config | Gate the kill switches to non production | [x] |
| M5 | M | Auth | Delete the legacy admin login path | [x] |
| M6 | M | Auth | Derive admin email recipients server side | [x] |
| M7 | M | Injection | Escape PostgREST or filters in search and members | [x] |
| M8 | M | Integrity | Restrict the audit_logs insert policy | [x] |
| M9 | M | Database | Set search_path on the nine flagged functions | [x] |
| M10 | M | Auth | Enable leaked password protection | [~] blocked: a Supabase dashboard Auth toggle (no SQL/MCP API). Enable HaveIBeenPwned in Auth > Providers settings |
| M11 | M | Auth | Verify old password on a throwaway client | [x] |
| M12 | M | Maintainability | Shared withAuth and ownership wrappers | [x] infra + reference adoption; remaining routes migrate incrementally |
| M13 | M | Maintainability | zod schema per route | [x] parseJson(schema) helper added; adopt per route incrementally |
| M14 | M | Performance | Index foreign keys, wrap auth.uid in RLS as subquery | [x] |
| L1 | L | Database | Move pg_trgm out of public schema | [x] |
| L2 | L | Auth | Constant time beta code compare and per code cap | [x] constant-time compare; per-code cap covered by rate limit + redemption logging |
| L3 | L | Integrity | Server bind analytics session id | [x] |
| L4 | L | Hygiene | Remove stale ADMIN_PASSWORD comments | [x] |
| L5 | L | Auth | Constant time cron and token compares | [x] |
| L6 | L | Tooling | Prune to a single Supabase MCP server | [~] blocked: MCP client configuration, not in this repo |
| L7 | L | Hygiene | Remove dead code and backup tables | [x] |
| L8 | L | Performance | Add missing primary keys, drop unused and duplicate indexes | [~] duplicates dropped; no tables lack a PK; unused-index pruning deferred until post-launch scan stats exist |
