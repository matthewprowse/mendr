...
- [x] Group A, resolved (June 2026). The "7 red files" were NOT all the dormant feature, which is why a blanket delete was avoided. Categorised and handled: (a) 3 genuinely-dormant tests deleted (agent-classify-facets, v2-prompt-drift, failure-modes); (b) 2 LIVE-code tests fixed, not deleted: agent-prose-errors (its mock had forgotten to export getDiagnosisModelByName) and agent-prose-parser (asserted pre-v7.4 behaviour; the parser deliberately stopped substituting the fallback for short thoughts to kill a 30% silent-fallback bug, so the test was stale); (c) 2 dead-code items removed: prompts/taxonomy-serializer.ts (no live importer, referenced an undefined EXCLUDED_SERVICES) with its test, and the src/__tests__/diagnostic-accuracy failure-modes eval harness. Result: the whole vitest suite is GREEN (190 files, 1951 tests, 0 failures, 0 skips). The diagnostic-accuracy eval flagged one real fixture/taxonomy drift before removal (a fixture expecting 'Solar & Backup Power' resolved to 'Electrical') — worth a look if an accuracy eval is rebuilt.
...
- [~] Whole test suite is GREEN at runtime (190 files, 1951 tests, 0 failures). tsc --noEmit still reports 81 errors, down from ~130: ~50 are the dormant diagnosis PRODUCTION code (failure-mode-serializer dormant parts, agent-reasoning, agent-critique, recommended-action, the facets refs in agent-prose/agent-classify) plus the diagnostic-reasoning eval (green but type-noisy), and ~17 are unrelated demo pages and a dead ui.backup dir. Removing the dormant production code is a surgical follow-up (agent-prose imports the live half of failure-mode-serializer, so the dead half must be split out, not rm'd).
