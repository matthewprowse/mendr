...
export function buildUnsupportedHomeServiceMessage(serviceLabels: string[]): string {
    ...
    if (servicesLine) {
        return `Issue appears to be home-related, but this specific service is not currently supported on Mendr. Services currently available are: ${servicesLine}. If this seems incorrect, add more information below and send it so we can reassess and route it correctly.`;
    }
    return 'Issue appears to be home-related, but this specific service is not currently supported on Mendr. If this seems incorrect, add more information below and send it so we can reassess and route it correctly.';
}
