...
const LAST_CONV_KEY = 'mendr_last_conversation_id';
const REPORTS_KEY = 'mendr_my_reports';
const LEGACY_REPORTS_KEY = 'scandio_my_reports';
...
export async function resolveWhatsAppPrefill(profileUrl: string): Promise<WhatsAppPrefill> {
    ...
    if (!report_url) {
        try {
            const raw = localStorage.getItem(REPORTS_KEY) ?? localStorage.getItem(LEGACY_REPORTS_KEY);
