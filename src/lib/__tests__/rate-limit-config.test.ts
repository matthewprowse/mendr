...
import { getCallerIp, isRateLimitBypassed, killSwitchActive, RATE_LIMITS } from '@/lib/rate-limit-config';
...
describe('killSwitchActive (M4)', () => {
    ...
    it('honours the kill switch outside production', () => {
        env.NODE_ENV = 'development';
        process.env.DISABLE_RATE_LIMIT = 'true';
        expect(killSwitchActive('DISABLE_RATE_LIMIT')).toBe(true);
    });

    it('ignores the kill switch in production', () => {
        env.NODE_ENV = 'production';
        process.env.DISABLE_RATE_LIMIT = 'true';
        expect(killSwitchActive('DISABLE_RATE_LIMIT')).toBe(false);
    });

    it('is inactive when the env var is unset', () => {
        expect(killSwitchActive('DISABLE_DIAGNOSIS_DAILY_QUOTA')).toBe(false);
    });
});
