...
function resetStore(): Map<string, unknown> {
    ...
    g.__MENDR_RATE_LIMIT_STORE__ = fresh;
    return fresh;
}
...
// ---------------------------------------------------------------------------
// M3 — fail closed in production for cost-bearing buckets without Upstash
// ---------------------------------------------------------------------------

describe('applyRateLimit — M3 fail-closed', () => {
    ...
    it('fails closed for a cost-bearing bucket when Upstash is unavailable', async () => {
    ...
    it('still degrades to memory for a non-cost bucket', async () => {
        const { applyRateLimit } = await import('@/lib/rate-limit');
        const res = await applyRateLimit({
            ip: '1.1.1.1',
