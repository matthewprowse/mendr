...
export const RATE_LIMITS = {
    ...
    // LLM cost estimate. One generation per diagnosis, then served from cache;
    // this caps how fast new diagnoses can trigger fresh generations per client.
    costEstimate: {
        windowMs: 60 * 1000, // 1 minute
        max: 30,
    },
    ...
    // ── WhatsApp Phase C ───────────────────────────────────────────────────────
    // Cloud API webhook. Keyed by IP (Meta's edges), so this is per-edge burst
    // protection, not per-user throttling — the HMAC signature check is the
    // real gate. 100/min respects the launch audit ceiling; Meta batches
    // multiple messages per delivery and fans out across edge IPs, so this is
    // ample at launch volumes. Revisit post-launch if 429s appear in logs.
    whatsappWebhook: {
        windowMs: 60 * 1000, // 1 minute
        max: 100,
    },

    // Magic-link consumption (browser GET). One legitimate click, maybe a retry.
    whatsappLink: {
        windowMs: 60 * 1000, // 1 minute
        max: 10,
    },

    // OTP send/verify — each send costs a template message; keep tight.
    whatsappOtp: {
        windowMs: 10 * 60 * 1000, // 10 minutes
        max: 6,
    },
...
/**
 * A protection kill switch (finding M4). These globally disable a safeguard, so
 * they are honoured ONLY outside production — a stray prod env value can no
 * longer turn everything off — and log loudly whenever they take effect.
 */
export function killSwitchActive(name: 'DISABLE_RATE_LIMIT' | 'DISABLE_DIAGNOSIS_DAILY_QUOTA'): boolean {
    if (process.env[name] !== 'true') return false;
    if (process.env.NODE_ENV === 'production') {
        console.error(
            `[security] ${name}=true ignored in production — protection kill switches are non-prod only.`,
        );
        return false;
    }
    console.warn(`[security] ${name}=true — protection disabled (non-production only).`);
    return true;
}

/**
 * Temporary local/dev bypass:
 * - DISABLE_RATE_LIMIT=true (global bypass, non-production only — see M4)
 * - RATE_LIMIT_BYPASS_IPS=127.0.0.1,::1 (IP-specific bypass)
 */
export function isRateLimitBypassed(req: NextRequest): boolean {
    if (killSwitchActive('DISABLE_RATE_LIMIT')) return true;
