...
/**
 * Conservative non-English greeting detection (Afrikaans / isiXhosa / isiZulu).
 * Used ONLY at idle on short greeting-like messages to reply gracefully in
 * English rather than misclassify — full multilingual support is a
 * fast-follow. Deliberately narrow: a false positive here would block a real
 * diagnosis request, which is far worse than missing a greeting.
 */
export function detectNonEnglishGreeting(textRaw: string): boolean {
    const t = textRaw.toLowerCase().trim();
    if (!t || t.length > 60) return false;
    return /(^|\s)(goeie\s?(môre|more|middag|naand)|hallo daar|asseblief|dankie|kan jy my help|praat jy|molo|molweni|ndicela|uxolo|sawubona|sanibonani|ngicela|unjani|uyakwazi)(\s|$|\?|!|,)/.test(
        t,
    );
}
