...
function rowToSession(row: SessionRow): WhatsappSession {
    return {
        ...
        resume_prompted_at: row.resume_prompted_at ?? null,
    };
}

const SELECT_COLS =
    'id, phone_number, user_id, state, active_diagnosis_id, pending_contractors, pending_address, pending_clarification, last_message_at, created_at, resume_prompted_at';
