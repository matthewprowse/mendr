...
export function formatContractorList(
    ...
    if (opts.hasMore) {
        lines.push('Reply MORE to see more options.');
    }
    return capMessage(lines.join('\n'));
}
...
/**
 * Registration gate for unknown numbers. When a magic link is supplied the
 * phone is verified by possession the moment they open it — two taps, no OTP.
 * Falls back to the plain register URL if the link could not be minted.
 */
export function formatRegistrationGate(magicLink?: string | null): string {
    if (magicLink) {
        return `Welcome to Mendr. To get your free diagnosis, tap this link to connect your WhatsApp — it takes two taps: ${magicLink}`;
    }
...
/** Response to "stop" — proactive messages are suppressed until START. */
export function formatStop(): string {
    return 'Okay, I will stop here and will not send you any more updates. Reply START any time to switch them back on, or just message me with a photo and I will help right away.';
}
