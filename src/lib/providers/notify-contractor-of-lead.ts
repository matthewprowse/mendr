...
export async function notifyContractorOfLead(input: {
    ...
    // 9. Send.
    // severity ('low' | 'medium' | 'high' | null) → template urgency scale.
    const urgency: NewLeadNotificationEmailProps['urgency'] =
        severity === 'medium' ? 'moderate' : (severity ?? 'low');

    const emailProps: NewLeadNotificationEmailProps = {
        contractorFirstName: contractorName,
        homeownerSuburb:     suburb ?? 'your area',
        faultTitle:          diagnosisTitle,
        faultCategory:       trade,
        urgency,
        leadUrl:             reportUrl,
        unsubscribeUrl,
        whatsappUrl:         whatsappDeeplink ?? undefined,
    };
    ...
    const result = await sendMendrEmail({
        ...
        subject,
        component: React.createElement(NewLeadNotificationEmail, emailProps),
        text: newLeadNotificationText(emailProps),
    });
