...
type SendArgs = Parameters<typeof import('@/lib/email').sendMendrEmail>;
type SendResult = Awaited<ReturnType<typeof import('@/lib/email').sendMendrEmail>>;
const sendMendrEmailMock = vi.fn(async (..._args: SendArgs): Promise<SendResult> => ({
    ok: true,
}));
vi.mock('@/lib/email', async () => {
    ...
    return {
        ...actual,
        sendMendrEmail: (...args: SendArgs) => sendMendrEmailMock(...args),
    };
});
...
beforeEach(() => {
    ...
    sendMendrEmailMock.mockClear();
    sendMendrEmailMock.mockResolvedValue({ ok: true });
});
...
describe('notifyContractorOfLead', () => {
    it('returns { ok: false, reason: "inactive" } when provider is not active', async () => {
        ...
        expect(sendMendrEmailMock).not.toHaveBeenCalled();
    });
    ...
    it('returns { ok: true } and sends an email when all conditions are met', async () => {
        ...
        expect(sendMendrEmailMock).toHaveBeenCalledTimes(1);
        const payload = sendMendrEmailMock.mock.calls[0]![0] as {
            to: { email: string };
            subject: string;
        };
    ...
    it('builds the wa.me deeplink correctly when homeownerWhatsapp is provided', async () => {
        ...
        const payload = sendMendrEmailMock.mock.calls[0]![0] as {
            text: string;
            component: { props: { whatsappUrl?: string } };
        };
        // Should normalise 0XXXXXXXXX → 27XXXXXXXXX.
        expect(payload.text).toContain('https://wa.me/27821234567');
        expect(payload.component.props.whatsappUrl).toContain('https://wa.me/27821234567');
    });
    ...
    it('omits the wa.me deeplink when homeownerWhatsapp is null', async () => {
        ...
        expect(payload.component.props.whatsappUrl).toBeUndefined();
    });
