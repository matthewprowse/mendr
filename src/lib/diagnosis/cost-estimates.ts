...
/**
 * Returns a human-readable cost estimate for the given subcategory_id.
 * Returns null when no estimate is available (e.g. none_unmapped).
 */
export function getCostEstimate(subcategoryId: string | null | undefined): FormattedCostEstimate | null {
    ...
    let label: string;
    if (est.max == null) {
        label = `From ${fmt(est.min)}`;
    } else {
        label = `${fmt(est.min)} – ${fmt(est.max)}`;
    }

    const unitPart = est.unit ? ` · ${est.unit}` : '';
    label = label + unitPart;
