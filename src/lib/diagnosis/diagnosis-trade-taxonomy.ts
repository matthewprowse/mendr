...
/**
 * A recognised failure mode within a subcategory (Bucket B structured domain
 * knowledge). Serialised into Agent 2b's prompt by `failure-mode-serializer`.
 */
export interface FailureMode {
    readonly id: string;
    readonly label: string;
    readonly description: string;
    readonly diagnosticCues: ReadonlyArray<{ readonly type: string; readonly description: string }>;
...
export interface TaxonomySubcategory {
    ...
    /**
     * Known failure modes for this subcategory (Bucket B). Optional — subcategories
     * still in rollout have none, and the serialiser falls back to general reasoning.
     */
