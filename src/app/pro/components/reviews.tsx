...
import { MENDR_CATEGORY_ROWS, REVIEWS_PAGE_SIZE } from '../lib/constants';
...
export function ProReviewsTab(props: {
    ...
    mendrReviewsCount: number;
    googleReviewsCount: number;
    mendrCategoryAggregates: Record<CategoryKey, number | null>;
    ...
    mendrReviewsShown: ReviewCard[];
    googleReviewsShown: ReviewCard[];
    mendrReviewCardsLength: number;
    googleReviewCardsLength: number;
    mendrReviewsVisibleCount: number;
    googleReviewsVisibleCount: number;
    setMendrReviewsVisibleCount: React.Dispatch<React.SetStateAction<number>>;
    ...
    return (
        <div className="flex flex-col gap-6 mt-2">
            ...
            {(isReviewsLoading || mendrReviewsCount > 0 || isOperatingHoursLoading) && (
                <div className="grid grid-cols-2 gap-2 lg:grid-cols-4 lg:gap-3">
                    {isReviewsLoading || isOperatingHoursLoading
                        ? MENDR_CATEGORY_ROWS.map((row) => (
                        ...
                        : MENDR_CATEGORY_ROWS.map((row) => {
                              const avg = mendrCategoryAggregates[row.key];
            ...
            <div className="flex flex-col gap-6">
                <div className="flex flex-col gap-3">
                    <div className="flex flex-row items-center justify-between gap-3">
                        ...
                        {isReviewsLoading || isOperatingHoursLoading ? (
                            ...
                            <Badge variant="secondary">{mendrReviewsCount}</Badge>
                        )}
                    </div>
                    <Button
                        type="button"
                ...
                {isReviewsLoading || isOperatingHoursLoading ? (
                    Array.from({ length: 2 }).map((_, i) => (
                        <div key={`mendr-review-skeleton-${i}`} className={reviewCardClass}>
                ...
                ) : mendrReviewsShown.length > 0 ? (
                    <>
                        {mendrReviewsShown.map((r) => (
                        ...
                        <ReviewsPaginationFooter
                            showingCount={mendrReviewsVisibleCount}
                            total={mendrReviewCardsLength}
                            categoryLabel="Mendr Reviews"
                            onViewMore={() =>
                                setMendrReviewsVisibleCount((prev) => Math.min(prev + REVIEWS_PAGE_SIZE, mendrReviewCardsLength))
                            }
                        />
                    </>
                ) : (
