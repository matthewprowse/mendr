...
export default function ProForgotPasswordClient() {
    const router = useRouter();
    const searchParams = useSearchParams();
    const next = searchParams.get('next') || '/pro/network';
        ...
        <FlowTopBar
            ...
            rightSlot={
                <Button asChild variant="ghost" size="sm">
                    <Link href={`/pro/auth/login${next !== '/pro/network' ? `?next=${encodeURIComponent(next)}` : ''}`}>
                        Login
                    </Link>
                </Button>
            }
