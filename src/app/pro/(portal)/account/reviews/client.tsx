    ...
    return (
        <span aria-label={rating != null ? `${rating}/5 stars` : 'No rating'} className="inline-flex">
            {Array.from({ length: 5 }).map((_, i) => (
                <span key={i} aria-hidden="true" className={i < filled ? 'text-amber-500' : 'text-muted-foreground/30'}>
                    ★
                </span>
            ))}
        </span>
...
function ReplyForm({
    ...
    async function submit() {
        ...
        setError(null);
        try {
            const res = await fetch(`/api/pro/reviews/${review.id}/reply`, {
                method: 'POST',
    ...
    return (
        <div className="flex flex-col gap-2">
            <textarea
                ...
                className="w-full rounded-lg border border-input bg-background p-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
    ...
    return (
        <li className="flex flex-col gap-3 border-t border-input py-4 first:border-t-0 first:pt-0 last:pb-0">
            ...
            {!editing && review.contractorReply ? (
                <div className="ml-4 rounded-lg border border-input bg-muted p-3">
        ...
        <>
            <div className="flex w-full flex-col gap-3">
                ...
                <p className="text-sm text-muted-foreground">
                    Homeowner ratings from completed Mendr jobs. Replies appear publicly on
                    your profile.
                </p>
            </div>
