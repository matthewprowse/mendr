...
import { ProReviewsTab } from '@/app/pro/components/reviews';
import { ProGalleryTab } from '@/app/pro/components/gallery';
import { ProPageMap } from '@/app/pro/[id]/components/map';
import {
    useContractor,
    type ContractorHydratePayload,
} from '@/app/pro/hooks/use-contractor';
import { useProReviews } from '@/app/pro/hooks/use-reviews';
import { useProGallery } from '@/app/pro/hooks/use-gallery';
import { useSavedProvider } from '@/app/pro/hooks/use-saved-provider';
import type { CategoryKey } from '@/app/pro/lib/types';
        ...
        <main className="flex min-h-screen flex-col bg-background">
            ...
            <div className="mx-auto flex w-full max-w-xl flex-1 flex-col gap-8 p-4 pb-[calc(6rem+env(safe-area-inset-bottom))]">
                ...
                <IdentityCard
                    ...
                    mendrReviewCount={profile?.mendrReviewCount ?? null}
                />
                ...
                <ReviewsSection>
                    <ProReviewsTab
                        ...
                        mendrReviewsCount={reviewsState.mendrReviewTotalFromMendr}
                        googleReviewsCount={reviewsState.googleReviewTotalFromGoogle}
                        mendrCategoryAggregates={reviewsState.mendrCategoryAggregates}
                        resolvedProviderId={reviewsState.resolvedProviderId}
                        shareOpen={shareOpen}
                        setShareOpen={(open) => {
                            setShareOpen(open);
                        ...
                        mendrReviewsShown={reviewsState.mendrReviewsShown}
                        googleReviewsShown={reviewsState.googleReviewsShown}
                        mendrReviewCardsLength={reviewsState.mendrReviewCards.length}
                        googleReviewCardsLength={reviewsState.googleReviewCards.length}
                        mendrReviewsVisibleCount={reviewsState.mendrReviewsVisibleCount}
                        googleReviewsVisibleCount={reviewsState.googleReviewsVisibleCount}
                        setMendrReviewsVisibleCount={reviewsState.setMendrReviewsVisibleCount}
