...
export async function POST(req: NextRequest): Promise<NextResponse> {
    ...
    // Owner override — constant-time compare so the master code can't be
    // recovered by timing (L2). Per-code brute-force is bounded by the betaAccess
    // rate-limit bucket above and the redemption logging in the RPC below.
    if (constantTimeEqual(submitted, master)) {
        return grant(deviceId, isNewDevice);
    }
