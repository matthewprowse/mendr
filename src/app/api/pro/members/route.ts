...
export async function POST(req: NextRequest): Promise<NextResponse> {
    ...
    // Structured equality checks rather than a string-built `.or()` so the
    // email value cannot inject filter syntax (finding M7).
    const baseDupe = () =>
        admin
            .from('provider_members')
            .select('id, status')
    ...
    const { data: dupeByEmail } = await baseDupe().eq('invited_email', email).limit(1);
    let dupe = dupeByEmail ?? [];
    if (dupe.length === 0 && existingUserId) {
        const { data: dupeByUser } = await baseDupe().eq('user_id', existingUserId).limit(1);
        dupe = dupeByUser ?? [];
    }
