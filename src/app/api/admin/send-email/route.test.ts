    ...
    supabase = mockSupabaseClient({
        tables: {
            provider_applications: {
                data: { email: 'pro@example.com', contact_name: 'Pro Co' },
                error: null,
            },
        },
    });
...
describe('POST /api/admin/send-email', () => {
    ...
    it('returns 404 when the application is not found', async () => {
        supabase = mockSupabaseClient({ tables: { provider_applications: { data: null, error: null } } });
        const { POST } = await import('./route');
        const res = await POST(makeRequest({ method: 'POST', body: validBody }));
    ...
    it('returns ok on success and derives the recipient server-side (M6)', async () => {
        const sendMock = vi.fn(async () => ({ data: { id: 'x' }, error: resendError }));
        const resend = await import('resend');
        // Vitest 4 requires a function (not arrow) for constructor mocks (`new Resend()`)
        vi.mocked(resend.Resend).mockImplementation(
            function (this: object) {
                return { emails: { send: sendMock } } as never;
            } as never,
        );
        const { POST } = await import('./route');
        // Client supplies an attacker address; it must be ignored.
        const res = await POST(
            makeRequest({ method: 'POST', body: { ...validBody, email: 'attacker@evil.test' } }),
        );
        expect(res.status).toBe(200);
        expect(sendMock).toHaveBeenCalledWith(
            expect.objectContaining({ to: 'Pro Co <pro@example.com>' }),
        );
    });
});
