...
/**
 * Resume / delete the caller's own in-progress application.
 *
 * Finding C6: this used to look an application up (and delete it) by a
 * client-supplied phone or id, or a spoofable x-forwarded-for header, returning
 * the full PII row to anyone who guessed a phone. Personal-data retrieval and
 * deletion are now keyed strictly on the authenticated session user_id — never
 * on client-supplied input. Anonymous callers get nothing.
 */
