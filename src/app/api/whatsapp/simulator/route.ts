...
/**
 * The simulator drives the real bot handler — impersonating any `from` phone,
 * reading back that phone's session, and burning Gemini quota. It is a
 * development tool, so it is disabled in production unless explicitly enabled
 * via ENABLE_WHATSAPP_SIMULATOR=true (finding M1).
 */
function simulatorDisabled(): boolean {
    return (
        process.env.NODE_ENV === 'production' &&
        process.env.ENABLE_WHATSAPP_SIMULATOR !== 'true'
    );
}
