...
describe('GET /api/contractors/application/edit', () => {
    it('returns 401 when token missing', async () => {
        const { GET } = await import('./route');
        const res = await GET(makeRequest({ path: '/api/contractors/application/edit' }));
    ...
    it('returns 401 when token revoked', async () => {
        ...
        const { GET } = await import('./route');
        const res = await GET(
            makeRequest({ path: `/api/contractors/application/edit?token=${VALID_TOKEN}` }),
        );
        expect(res.status).toBe(401);
    });
...
describe('POST /api/contractors/application/edit', () => {
