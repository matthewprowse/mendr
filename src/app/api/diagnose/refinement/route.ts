...
import { checkRateLimit, isRateLimitBypassed, killSwitchActive } from '@/lib/rate-limit-config';
...
export async function POST(req: NextRequest): Promise<NextResponse> {
    ...
    // Honoured only outside production (finding M4).
    if (killSwitchActive('DISABLE_DIAGNOSIS_DAILY_QUOTA') || isRateLimitBypassed(req)) {
