...
vi.mock('@/lib/rate-limit-config', () => ({
    ...
    killSwitchActive: vi.fn((name: string) => process.env[name] === 'true'),
}));
...
describe('checkDiagnosisQuota — anonymous cookie', () => {
    it('issues a Set-Cookie when no anon cookie present', async () => {
        rpcSpy.mockResolvedValueOnce({ data: 1, error: null });
        const { checkDiagnosisQuota } = await import('../quota');
        const result = await checkDiagnosisQuota({ req: makeReq(), body: {} });
        expect(result.blockingResponse).toBeNull();
        expect(result.extraHeaders['Set-Cookie']).toMatch(/^mendr_anon=/);
    });
    ...
    it('keys the anonymous quota on the client IP, not the cookie (H3)', async () => {
        rpcSpy.mockResolvedValueOnce({ data: 1, error: null });
        const { checkDiagnosisQuota } = await import('../quota');
        await checkDiagnosisQuota({
            req: makeReq({ 'x-forwarded-for': '41.2.3.4, 10.0.0.1' }),
            body: {},
        });
        expect(rpcSpy).toHaveBeenCalledTimes(1);
        const args = rpcSpy.mock.calls[0][1] as { p_anon_key: string | null };
        // Hashed IP, prefixed — never the raw cookie UUID or the raw IP.
        expect(args.p_anon_key).toMatch(/^ip:[0-9a-f]{40}$/);
    });
});
