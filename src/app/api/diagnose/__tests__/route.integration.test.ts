...
describe('/api/diagnose — happy path (non-streaming, text-only)', () => {
    ...
    it('emits the X-Scandio-Ai-Model + X-Scandio-Prompt-Version response headers', async () => {
        const { POST } = await import('../route');
        const res = await POST(makeRequest({ textQuery: 'help' }));
        expect(res.headers.get('X-Scandio-Ai-Model')).toBeTruthy();
        expect(res.headers.get('X-Scandio-Prompt-Version')).toBeTruthy();
    });
});
