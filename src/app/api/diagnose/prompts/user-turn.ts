...
export function buildImageFollowUpPrompt(params: {
    ...
    return (
        ...
        ` Provide a FULL diagnosis: identify the equipment/issue, set diagnosis, action_required, estimated_cost, and trade. ` +
...
export function buildProviderHydrationImagePrompt(params: {
    ...
    return (
        params.instructionPrefix +
        params.userWordsPriority +
        `PROVIDER HYDRATION PASS: Re-read ${params.imageCount > 1 ? 'these images' : 'this image'} and output a full Scandio response. ` +
