 ...
 * - RESEND_API_KEY, RESEND_FROM (format: "Mendr <noreply@mendr.co.za>") // TODO(mendr-domain): mendr.co.za must be verified in Resend (SPF/DKIM/DMARC) before relying on this
 * - NEXT_PUBLIC_SUPABASE_URL
 * Optional: AUTH_EMAIL_PUBLIC_URL — public origin for `/fonts/AnthropicSans-Text-*.otf` (defaults to NEXT_PUBLIC_APP_URL or VERCEL_URL)
 */
export async function POST(req: NextRequest) {
    ...
    const fromName   = nameMatch ? nameMatch[1].trim() : (process.env.RESEND_FROM_NAME || 'Mendr');
