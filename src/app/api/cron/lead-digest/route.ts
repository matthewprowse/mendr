...
import { sendScandioEmail, leadDigestEmail } from '@/lib/resend-mail';
...
async function handler(req: NextRequest): Promise<NextResponse> {
    ...
    // Fetch unsent events in the target month window
    ...
    for (const [providerId, { count, trades }] of byProvider) {
        ...
        // Check if this provider has a registered application
        ...
        const { text, html } = leadDigestEmail({
            businessName,
            contactCount: count,
        ...
        const result = await sendScandioEmail({
            ...
            subject: `${count} homeowner contact${count === 1 ? '' : 's'} — ${monthLabel} | Menda`,
            text,
            html,
        });
