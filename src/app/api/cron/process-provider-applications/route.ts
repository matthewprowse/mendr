...
// Candidate providers are scored client-side, so we cap the fetch. If the table
// ever exceeds this, matching would silently consider only the first N rows —
// we log a warning when the cap is hit so the truncation is visible.
const CANDIDATE_FETCH_CAP = 1000;
...
async function processApplication(
    ...
    if (candidates.length === CANDIDATE_FETCH_CAP) {
        console.warn(
            `[process-applications] candidate fetch hit the ${CANDIDATE_FETCH_CAP}-row cap; ` +
                'some providers may not be considered for matching. Consider DB-side scoring.',
        );
    }
